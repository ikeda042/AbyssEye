...
from ..tiff_manager_bulk import crud as tiff_bulk_crud

UPLOAD_DIR = data_path("retraining_uploads")
RUNS_DIR = data_path("retraining_runs")
...
def _safe_dataset_relative_path(value: str, *, field: str) -> Path:
    cleaned = str(value or "").replace("\\", "/").strip()
    if not cleaned:
        raise HTTPException(status_code=400, detail=f"{field} が空です。")
    path = Path(cleaned)
    if path.is_absolute() or any(part in {"", ".", ".."} for part in path.parts):
        raise HTTPException(status_code=400, detail=f"{field} に不正なパスが含まれています。")
    ...
    try:
        with zipfile.ZipFile(archive_path, "r") as zf:
            ...
            for member in names:
                ...
                if not normalized_member.startswith(f"{DATASET_SUBDIR}/") or normalized_member.endswith("/"):
                    continue
                relative_raw = normalized_member[len(f"{DATASET_SUBDIR}/") :]
                relative = _safe_dataset_relative_path(relative_raw, field="ZIP内ファイル名")
                target_path = dataset_dir / relative
                try:
                    target_path.resolve().relative_to(dataset_dir.resolve())
                except ValueError as exc:
                    raise HTTPException(status_code=400, detail="ZIP内ファイル名が展開先の外を指しています。") from exc
...
def _load_dataset_examples(dataset_dir: Path) -> list[TrainingExample]:
    ...
    for row in rows:
        ...
        if label is None or label < 0 or label >= NUM_CLASSES or not relative_path:
            continue
        safe_relative_path = _safe_dataset_relative_path(relative_path, field="labels.csv relative_path")
        image_path = dataset_dir / safe_relative_path
        try:
            image_path.resolve().relative_to(dataset_dir.resolve())
        except ValueError as exc:
            raise HTTPException(status_code=400, detail="labels.csv relative_path がデータセット外を指しています。") from exc
