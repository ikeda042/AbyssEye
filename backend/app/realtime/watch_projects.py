...
def build_powershell_watch_script(project_name: str, api_url: str) -> str:
    ...
    lines = [
        ...
        "",
        "# 既存ファイルの現在状態を記録する",
        "$seenSignatures = New-Object 'System.Collections.Generic.Dictionary[string,string]'",
        "Get-ChildItem -Path $WatchPath -File | Where-Object {",
        '    $_.Extension.ToLower() -in @(".tif", ".tiff") -and $_.BaseName -notmatch "^TMP[0-9A-F]+$"',
        "} | ForEach-Object {",
        "    $signature = Get-FileSignature -FilePath $_.FullName",
        "    if ($null -ne $signature) {",
        "        $seenSignatures[$_.FullName] = $signature",
        "    }",
        "}",
        "",
        "try {",
        ...
        '            $_.Extension.ToLower() -in @(".tif", ".tiff") -and $_.BaseName -notmatch "^TMP[0-9A-F]+$"',
        "        } | ForEach-Object {",
        "            $currentSignature = Get-FileSignature -FilePath $_.FullName",
        "            if ($null -ne $currentSignature) {",
        "                $knownSignature = $null",
        "                if ($seenSignatures.ContainsKey($_.FullName)) {",
        "                    $knownSignature = $seenSignatures[$_.FullName]",
        "                }",
        "",
        "                if ($knownSignature -ne $currentSignature) {",
        "                    # 新しく見つかった TIFF ファイル、または更新された TIFF ファイル",
        "                    if (Send-TiffFile -FilePath $_.FullName -ApiUrl $ApiUrl) {",
        "                        $latestSignature = Get-FileSignature -FilePath $_.FullName",
        "                        if ($null -eq $latestSignature) {",
        "                            $latestSignature = $currentSignature",
        "                        }",
        "                        $seenSignatures[$_.FullName] = $latestSignature",
        "                    }",
        "                }",
        "            }",
        "        }",
